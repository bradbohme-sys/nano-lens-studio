...
import { LeftSettingsPanel } from "./settings/LeftSettingsPanel";
...
export const DSLRInterface = () => {
  ...
  const handleGenerate = async () => {
    const cameraSettings = {
      aperture, iso, shutterSpeed, exposure, contrast, saturation,
    };
    ...
    <div className="h-screen w-full bg-camera-body flex flex-col">
      <header className="camera-panel h-16 flex items-center justify-between px-6 border-b border-border">
        <div className="flex items-center gap-4">
          <div className="text-primary text-xl font-bold font-mono">AI CAMERA</div>
        ...
        <div className="flex items-center gap-4">
          <Textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Enter AI prompt..." className="w-64 h-10 resize-none text-sm" />
          <Button onClick={handleGenerate} disabled={isGenerating || !prompt.trim()} size="lg" className="camera-button font-mono font-semibold px-6">
            {isGenerating ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />GENERATING</> : <><Sparkles className="mr-2 h-5 w-5" />GENERATE</>}
          </Button>
          <div className="lcd-display px-3 py-1 rounded animate-lcd-glow"><span className="text-lcd">ISO {iso}</span></div>
          <div className="lcd-display px-3 py-1 rounded"><span className="text-lcd">f/{aperture}</span></div>
          <div className="lcd-display px-3 py-1 rounded"><span className="text-lcd">1/{shutterSpeed}</span></div>
        </div>
      </header>

      <div className="flex-1 flex">
        <LeftSettingsPanel activeTool={activeTool} onCollapse={setSettingsCollapsed} collapsed={settingsCollapsed} />
        <ToolSidebar activeTool={activeTool} onToolChange={setActiveTool} module={activeModule} />
        <div className="flex-1 flex flex-col">
          <MainCanvas activeTool={activeTool} isProcessing={isGenerating} generatedImageUrl={generatedImage} canvasRef={canvasRef} />
          <StatusBar isProcessing={isGenerating} activeTool={activeTool} />
        </div>
        <div className="flex">
          <RightPanelNav activePanel={activePanel} onPanelChange={setActivePanel} />
          <SpecialLayersBar />
          <div className="w-72 flex flex-col">
            {activePanel === 'camera' && <CameraControls aperture={aperture} iso={iso} shutterSpeed={shutterSpeed} exposure={exposure} contrast={contrast} saturation={saturation} onApertureChange={setAperture} onIsoChange={setIso} onShutterSpeedChange={setShutterSpeed} onExposureChange={setExposure} onContrastChange={setContrast} onSaturationChange={setSaturation} />}
            ...
            {activePanel === 'cursor-zoom' && <CursorZoomPanel canvasRef={canvasRef} />}
            {activePanel === 'microscope' && <MicroscopePanel canvasRef={canvasRef} activeTool={activeTool} />}
            {activePanel === 'color-sphere' && <ColorSpherePanel />}
            {activePanel === 'prompt-preview' && <PromptPreviewPanel cameraSettings={{ aperture, iso, shutterSpeed, exposure, contrast, saturation }} prompt={prompt} />}
